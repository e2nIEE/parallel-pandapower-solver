...
def net_with_trafo_characteristic() -> pandapowerNet:
    ...
    create_transformer(net, hv_bus=b2, lv_bus=cb, std_type="0.25 MVA 20/0.4 kV", tap_pos=-2, shift_degree=150,
                       id_characteristic_table=0, tap_dependency_table=True)
...
def test_p3s(networks: pandapowerNet):
    ...
    for net in networks:
        ...
        print(f"pp, {net2['_ppc']['iterations']} time: {round(pp_time, 3)}s "
