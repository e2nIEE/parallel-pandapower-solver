...
from p3s.contingency.case_generator import ContingencyCaseGenerator, ContingencyBatch
...
def main():
    ...
    t = time.perf_counter(); T["build_net"] = t - t0; t0 = t

    gen = ContingencyCaseGenerator(net, reslack_islands=False)
    batch: ContingencyBatch = gen.build()
    n = batch.n_bus
    L = len(batch.cases)
    t = time.perf_counter(); T["case_generator.build"] = t - t0; t0 = t
    ...
    t = time.perf_counter(); T["solver_init(kernels+topo)"] = t - t0; t0 = t

    v_start = (dc_initial_voltage(gen._npf) if args.init == "dc"
               else gen._npf._initial_voltage.copy())
    ...
    t = time.perf_counter(); T["assemble Yx (complex, no host abs/angle)"] = t - t0; t0 = t
    ...
    for c, case in enumerate(batch.cases):
        ...
        V0[:, c] = v0
    t = time.perf_counter(); T["build V0/pin (per-case loop)"] = t - t0; t0 = t

    # the actual solve (includes host->device transfer + cuDSS analysis + GPU solve)
    _ = sync_t()
    tS = time.perf_counter()
    res = solver.solve_batch_contingency_cx(Yx_mat, Sbus,
