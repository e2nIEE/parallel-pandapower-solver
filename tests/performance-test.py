...
from pandapower.networks.power_system_test_cases import case5, case9, case14, case118, case1354pegase, case2848rte, case6495rte, case9241pegase
from pandapower.toolbox.data_modification import create_continuous_bus_index
from pandapower.create import (
    create_empty_network, create_bus, create_ext_grid, create_line_from_parameters, create_load,
...
from util import compare_arrays
import cProfile
import pstats
...
def net_with_trafo_characteristic() -> pandapowerNet:
    ...
    create_line_from_parameters(net, b1, b2, 12.2, r_ohm_per_km=0.08, x_ohm_per_km=0.12,
                                        c_nf_per_km=300, max_i_ka=.2)
    cb = create_bus(net, vn_kv=0.4)
    create_load(net, cb, 0.2, 0.05)

    create_transformer_from_parameters(net, hv_bus=b2, lv_bus=cb, sn_mva=9900., vn_hv_kv=20, vn_lv_kv=0.4,
                                       vk_percent=2070.288, vkr_percent=0., pfe_kw=0., i0_percent=0., shift_degree=0.,
                                       tap_side='hv', tap_neutral=0., tap_step_percent=2.2, tap_pos=-1., in_service=True,
                                       max_loading_percent=100., tap_changer_type='Ratio')
...
if __name__ == "__main__":
    ...
    for name, net in nets.items():
        ...
        print(f"pp time[ms]: {(pp_end_time - pp_start_time)/N*1000.}, iterations: {pp_iter}, "
              f"gpu time[ms]: {(cu_end_time-cu_start_time)/N*1000.}, iterations: {cuda_iter}, "
              f"p3s time[ms]: {(p3s_end_time - p3s_start_time)/N*1000.}, iterations: {p3s_iter},")
        results.loc[name, 'pp'] = (pp_end_time - pp_start_time)/N*1000.
        results.loc[name, 'gpu'] = (cu_end_time-cu_start_time)/N*1000.
        results.loc[name, 'p3s'] = (p3s_end_time - p3s_start_time)/N*1000.
