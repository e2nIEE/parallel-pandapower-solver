...
def calculate_trafo_characteristic(net: pandapowerNet, inplace: bool = False):
    ...
    n_taps: NDArray = np.empty(len(net.trafo), dtype=float)
    tap: NDArray = np.empty(len(net.trafo), dtype=complex)
    ratios: NDArray = np.ones(shape=len(net.trafo))

    def _ideal(idx):
        n_taps[idx] = 2 * np.rad2deg((tap_pos[idx] - tap_neutral[idx]) * tap_step_percent[idx] / 200.0)
    ...
    def _symmetrical(idx):
        n_taps = -2 * np.rad2deg((tap_pos[idx] - tap_neutral[idx]) * tap_step_percent[idx] / 200.0)
        tap[idx] = np.exp(1j * np.deg2rad(-tap_step_degree[idx] + n_taps[idx]))
    ...
    # double-counting it.)
    tap_angle_deg = (tap_pos.fillna(0.0) - tap_neutral.fillna(0.0)) * tap_step_degree

    df = (
        pd.DataFrame(
            {
                "id_characteristic": range(len(net.trafo)),
                "step": tap_pos.fillna(0.0),
