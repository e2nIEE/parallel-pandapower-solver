...
from scipy.sparse import coo_matrix as sparse, vstack, hstack, csr_matrix
...
@jit(nopython=True, cache=False)
def _dSbus_dV_numba_faster_sparse(Yx: np.ndarray, Yp: np.ndarray, Yj: np.ndarray, Yd: np.ndarray, voltage: np.ndarray): # pragma: no cover
    Vnorm = np.abs(voltage)

    dS_dVm = np.zeros(len(Yx), dtype=np.complex128)
    dS_dVa = np.zeros(len(Yx), dtype=np.complex128)
...
class PQPVPowerflow(PowerflowObject):
    ...
    def __init__(self, YBus: sparse, pq: np.ndarray, pv: np.ndarray, ref: np.ndarray):
        super().__init__()
        self._YBus: sparse = YBus
        self._pv: np.ndarray = pv
        self._pq: np.ndarray = pq
        self._ref: np.ndarray = ref
        self._pvpq: np.ndarray = np.r_[pv, pq]
        self._cols_pvpq: np.ndarray = self._pvpq
        self._rows_pvpq: np.ndarray = self._pvpq.T
    ...
    def _dSbus_dV(self, V: np.ndarray):
        Ibus = self._YBus * V
        ib = range(len(V))
    ...
    def evaluate_Fx(self, Sbus: np.ndarray, V: np.ndarray):
