...
import numpy as np
import pandas as pd
from scipy.sparse import coo_matrix as sparse, coo_matrix, block_diag
...
from p3s.PQPVPowerflow import PQPVPowerflow, get_ybus_diag_ix
...
class NewtonPowerflow:
    def __init__(self, net):
        ...
        self._ybus_elements = dict()
        self.busses: dict[str, np.ndarray] = None
    ...
    def make_ybus(self, net: pandapowerNet) -> Tuple[sparse, sparse]:
        ...
        if "trafo" in net and len(net.trafo) > 0:
            trafos = TwoWindingTransformerModel(net.trafo, net.bus, sn_mva=net.sn_mva, tap_table=net.trafo_characteristic_table)
    ...
    def _setup_pf(self, net: pandapowerNet):
        ...
        # Lookup tables, since the Ybus is 0 indexed. Lookup is from bus to ybus index. Reverse_lookup is the other way around.
        ...
        if "sgen" in net and len(net['sgen']) > 0:
            ...
            _sgen = -1. * res_mw.groupby(net.sgen["_lookup"]).sum()
        ...
        if "gen" in net and len(net['gen']) > 0:
            ...
            _gen = -1. * res_mw.groupby(net.gen["_lookup"]).sum()
    ...
    def _parse_results(self, net, voltage):
        def _ensure_index(df: pd.DataFrame, index: pd.Index) -> pd.DataFrame:
            if len(df) != len(index) or not df.index.equals(index):
                return df.reindex(index, copy=False)
            return df
        ...
        if 'line' in self._ybus_elements and 'line' in net and len(net.line):
            ...
            res_line['pl_mw'].to_numpy(copy=False)[:] = np.abs(np.abs(line_power_to.real) - np.abs(line_power_from.real))
            res_line['ql_mvar'].to_numpy(copy=False)[:] = np.abs(np.abs(line_power_to.imag) - np.abs(line_power_from.imag))
        ...
        if 'trafo' in self._ybus_elements and 'trafo' in net and len(net.trafo):
            ...
            loading_percent = np.maximum(trafo_currents_from.values * trafos.voltages_from.values * np.sqrt(3),
        ...
        # res_gen['q_mvar'].to_numpy(copy=False)[:] = -1 * gen_bus.q_mvar.to_numpy(copy=False) - self._sBus[self.pf_objects['PVPQ']._pv].imag * net.sn_mva
    ...
    def _update_q(baseMVA, bus, gen, gbus, Sbus, on):
        ...
        net.gen.q_mvar = 0. # zero out all Qg
    ...
    def _pre_dc_solve(self, yBus: sparse, voltage: np.ndarray, Pinj: np.ndarray, ref, pvpq):
    ...
    def calculate(self, net: pandapowerNet, tolerance: float = 1e-5, max_iterations: int = 30, **kwargs):
        ...
        pv: np.ndarray = self.busses['pv']
        pq: np.ndarray = self.busses['pq']
        pvpq = np.r_[pv, pq]
        ...
        if initialize_pf == "dc":
            ...
            voltage[pvpq] = self._pre_dc_solve(yBus=self._Bbus, voltage=voltage,
                                               Pinj=self._sBus.real + self._p_shift,
                                               ref=self.busses['ref'], pvpq=pvpq)
        ...
        def _in_band(v: np.ndarray) -> bool:
        ...
        for V0_seed in seeds:
            result = self._cpp_solver.solve(Sbus, V0_seed, max_iter=max_iterations,
                                            tol=tolerance, line_search=line_search)
    ...
    def calculate_timeseries_cpp(self, net: pandapowerNet, timeseries: dict,
        ...
        result = self._cpp_solver.solve_batch(
            sbus_contiguous,
            v_init_contiguous,
            max_iter=max_iterations, tol=tolerance, n_threads=n_threads,
        )
        ...
        if not bool(np.all(result["converged"])):
            ...
            raise LoadflowNotConverged(
                f"C++ batch did not converge for {n_bad} of {T} time steps "
