...
class CudssBatch:
    ...
    def __init__(self, Jp, Jj, batch_size, reorder="symrcm",
                 numeric_zero=0.0, numeric_boost=0.0, pivot="none"):
        ...
        C.check(C._libcudss.cudssDataCreate(self._h, ctypes.byref(self._data)),
                "cudssDataCreate")
        ...
        C.check(C._libcudss.cudssMatrixCreateCsr(
            ctypes.byref(self._A),
            ctypes.c_int64(n), ctypes.c_int64(n), ctypes.c_int64(nnz),
            ctypes.c_void_p(int(self._d_Jp)), ctypes.c_void_p(0),
            ctypes.c_void_p(int(self._d_Jj)), ctypes.c_void_p(int(self._d_A_batch)),
            C.CUDA_R_32I, C.CUDA_R_32I, C.CUDA_R_64F,      # offsetType, indexType, valueType
            C.CUDSS_MTYPE_GENERAL, C.CUDSS_MVIEW_FULL, C.CUDSS_BASE_ZERO),
            "cudssMatrixCreateCsr")
        self._b = ctypes.c_void_p()
        C.check(C._libcudss.cudssMatrixCreateDn(
            ctypes.byref(self._b), ctypes.c_int64(n), ctypes.c_int64(1), ctypes.c_int64(n),
            ctypes.c_void_p(int(self._d_rhs_batch)), C.CUDA_R_64F, C.CUDSS_LAYOUT_COL_MAJOR),
            "cudssMatrixCreateDn(b)")
        self._x = ctypes.c_void_p()
        C.check(C._libcudss.cudssMatrixCreateDn(
            ctypes.byref(self._x), ctypes.c_int64(n), ctypes.c_int64(1), ctypes.c_int64(n),
            ctypes.c_void_p(int(self._d_X_batch)), C.CUDA_R_64F, C.CUDSS_LAYOUT_COL_MAJOR),
        ...
        ub = ctypes.c_int(B)
        C.check(C._libcudss.cudssConfigSet(
            self._cfg, C.CUDSS_CONFIG_UBATCH_SIZE,
            ctypes.byref(ub), ctypes.c_size_t(ctypes.sizeof(ub))),
        ...
        if self.pivot == "none":
            ...
            C.check(C._libcudss.cudssConfigSet(
                self._cfg, C.CUDSS_CONFIG_PIVOT_TYPE,
                ctypes.byref(pv), ctypes.c_size_t(ctypes.sizeof(pv))),
    ...
    def _execute(self, phase, what):
        C.check(C._libcudss.cudssExecute(
            self._h, phase, self._cfg, self._data, self._A, self._x, self._b), what)
    ...
    def free(self):
        ...
        try:
            if getattr(self, "_data", None) is not None and self._data.value:
                C._libcudss.cudssDataDestroy(self._h, self._data); self._data = None
            if getattr(self, "_cfg", None) is not None and self._cfg.value:
                C._libcudss.cudssConfigDestroy(self._cfg); self._cfg = None
            if getattr(self, "_h", None) is not None and self._h.value:
                C._libcudss.cudssDestroy(self._h); self._h = None
        except Exception:
            pass
