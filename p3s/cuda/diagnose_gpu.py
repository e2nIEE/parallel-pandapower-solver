...
def main():
    ...
    print(f"CUDA_HOME   : {os.environ.get('CUDA_HOME') or os.environ.get('CUDA_PATH')}",
          flush=True)
    ...
    if ndev == 0:
        print("  FATAL: no CUDA devices visible.\n"
              "  -> check CUDA_VISIBLE_DEVICES and that the job actually allocated a GPU "
              "(e.g. srun --gres=gpu:1). pycuda.autoinit would segfault here.", flush=True)
        return
    for i in range(ndev):
        d = cuda.Device(i)
        cc = d.compute_capability()
        print(f"    dev {i}: {d.name()}  cc={cc[0]}.{cc[1]}  "
              f"{d.total_memory() // (1024**2)} MB", flush=True)
    ...
    need = ["cusolverSpCreate", "cusolverSpCreateCsrluInfoHost",
            "cusolverSpDcsrluExtractHost", "cusolverSpXcsrluNnzHost",
            "cusolverRfCreate", "cusolverRfBatchSetupHost",
            "cusolverRfBatchRefactor", "cusolverRfBatchSolve"]
    missing = [s for s in need if not hasattr(W._libcusolver, s)]
    if missing:
        print(f"  FATAL: cuSolver lib is MISSING symbols {missing}.\n"
              ...
              "(CUDA 11.x, or a 12.x that retained cusolverRf).", flush=True)
        return
    ...
    for k in ("eval_F_and_J_polar", "gather_J_polar", "negate_F",
              "update_voltage_polar", "inf_norm_per_column"):
        mod.get_function(k)
...
def _tiny_cudss_solve():
    ...
    try:
        ...
        print(f"  SKIP: libcudss not loadable ({e}). pip install nvidia-cudss-cu12 to use "
              "backend='cudss'.", flush=True)
        return
    ver = C.version()
    if ver:
    ...
    Jx = np.array([4., 1., 1., 4., 1., 1., 4.], dtype=np.float64)
    ...
    s.free()
    if err < 1e-9:
        print(f"  PASS: cuDSS 3x3 batch solve OK (err {err:.1e}) -> backend 'cudss' usable",
              flush=True)
    else:
...
def _tiny_qr_solve():
    ...
    d_Jp = cuda.mem_alloc(Jp.nbytes); cuda.memcpy_htod(d_Jp, Jp)
    d_Jj = cuda.mem_alloc(Jj.nbytes); cuda.memcpy_htod(d_Jj, Jj)
    d_Jx = cuda.mem_alloc(Jx.nbytes); cuda.memcpy_htod(d_Jx, Jx)
    ...
    st = _libcusolver.cusolverSpDcsrlsvqr(
        spH, n, nnz, descr,
        ctypes.c_void_p(int(d_Jx)), ctypes.c_void_p(int(d_Jp)),
        ctypes.c_void_p(int(d_Jj)), ctypes.c_void_p(int(d_b)),
        ctypes.c_double(0.0), ctypes.c_int(0),
        ctypes.c_void_p(int(d_x)), ctypes.byref(singular))
    cuda.Context.synchronize()
    if st != 0:
        raise RuntimeError(f"cusolverSpDcsrlsvqr status {st}")
    x = np.empty(3, np.float64); cuda.memcpy_dtoh(x, d_x)
    A = np.array([[4, 1, 0], [1, 4, 1], [0, 1, 4]], float)
    err = np.linalg.norm(x - np.linalg.solve(A, b), np.inf)
    assert err < 1e-9, f"QR 3x3 wrong: err {err:.2e}"
    _libcusparse.cusparseDestroy if False else None
...
def _tiny_solve():
    ...
    yb = npf._YBus.tocsr(); yb.sort_indices()
    Yp = yb.indptr.astype(np.int32); Yj = yb.indices.astype(np.int32)
