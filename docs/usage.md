...
`pandapowerNet` and writes pandapower-style `res_*` tables back into it.
...
With `inplace=True` the result is written to `net.trafo_characteristic_table`;
with the default `inplace=False` it is returned instead. Call this before any of
the solvers below for networks that contain trafos.

## Newton-Raphson solver (`p3s.NewtonPowerflow`)
...
- `init="dc"` (default) initialises the voltage vector with a DC power flow; use
  `init="flat"` for a 1.0 pu / 0° flat start. DC init preserves the voltage-magnitude
  set-points at PV buses.
- `tolerance` (default `1e-5`) is the convergence threshold (infinity norm of the
  power mismatch);
- `max_iterations` defaults to `30`.

The solver writes results back into the network (`net.res_bus.vm_pu`,
`net.res_bus.va_degree`, plus the other `res_*` tables), matching pandapower
`runpp`, and also returns the converged voltage vector `voltage`. Any extra keyword
arguments are passed through to `scipy.sparse.linalg.spsolve`.

The number of iterations taken is stored in `net["_ppc"]["iterations"]`.

## C++ / KLU accelerated solver (`p3s.NewtonPowerflowCpp`)
...
`n_threads=0` (default) uses all available threads; set it to `1` for a single
thread. See [Time-series data](#time-series-data) for the `timeseries` format.

## GPU solver (`p3s.cuda.NewtonPowerflowCUDA`)
...
The batched solvers (`calculate_timeseries_cpp`, `calculate_timeseries_cuda`, ...)
...
## N-1 contingency analysis (`p3s.contingency`)
...
`reslack_islands=True` re-slacks islands that form after an outage, so that the
...
`reslack_islands=True` same as before, re-slacks islands that form after an outage, so that the
